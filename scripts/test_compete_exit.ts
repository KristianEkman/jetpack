...
try {
  ...
  let client1LevelComplete: any = null;
  let client2LevelComplete: any = null;

  client1.on(GAME_EVENTS.LEVEL_COMPLETE, (data: any) => {
    client1LevelComplete = data;
  });
  client2.on(GAME_EVENTS.LEVEL_COMPLETE, (data: any) => {
  ...
  let client1GameStarted: any = null;
  let client2GameStarted: any = null;

  client1.on(GAME_EVENTS.GAME_STARTED, (data: any) => {
    client1GameStarted = data;
  });
  client2.on(GAME_EVENTS.GAME_STARTED, (data: any) => {
    client2GameStarted = data;
  });

  const nextResult: any = await new Promise((resolve) => {
