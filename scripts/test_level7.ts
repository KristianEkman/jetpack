...
if (stage7.bosses) {
    stage7.bosses.forEach(b => enemyManager.addBoss(b.x, b.y, b.hp || 35));
}
...
assert.equal(bossEnemies[0].hp, 35, "Stage 7 Boss HP should be 35");
assert.equal(bossEnemies[0].maxHp, 35, "Stage 7 Boss Max HP should be 35");

assert.equal(enemyManager.enemies.length, 6, "Total enemies for Stage 7 should be 6 (1 Boss, 2 Flitzers, 1 Missile, 2 Turrets)");
console.log("3️⃣  Enemy spawner verified with Boss (35 HP) and supporting artillery.");
