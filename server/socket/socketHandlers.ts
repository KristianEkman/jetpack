...
import type { RoomManager, ServerRoom } from "../roomManager.js";
...
import { TILES } from "../../js/world/tilemap.js";
...
export const ROOM_LIST_CHANNEL = "__room_list_watchers__";

export function broadcastRoomList(io: Server, roomManager: RoomManager): void {
  io.to(ROOM_LIST_CHANNEL).emit("room_list_updated", roomManager.listRooms());
}

export function initRoomEnemies(
  room: ServerRoom,
...
/**
 * Initializes level map, spawns, and player states when starting a match or advancing to next level.
 */
  ...
  io.on("connection", (socket: Socket) => {
    ...
    socket.on(
      ...
      (
        ...
        const payload = launchLevel(room, roomManager, gameLoop);
